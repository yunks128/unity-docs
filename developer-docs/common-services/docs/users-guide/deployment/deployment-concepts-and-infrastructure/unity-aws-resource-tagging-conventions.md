...
| Type   | Example Values                         | Required | Default Value      |
| ------ | -------------------------------------- | -------- | ------------------ |
| String | `sps`, `ds`, `as`, `ads`, `cs`, `uiux` | True     | N/A - User Defined |

* Description: The name of the Unity service area.
* Pattern: `{ServiceArea}`
* Pattern Components:
  * **ServiceArea**:`(cs\|sps\|ds\|ads\|as\|uiux)`|See `ServiceArea` for more info. The service area abbreviation, or "all" if the resource is to be used by all service areas
* Pattern Example:
  * `sps`
